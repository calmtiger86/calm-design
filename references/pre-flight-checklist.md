# Pre-Flight Checklist — 33항목 출력 검증

> **Inspired by**: [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill) — Pre-Flight Checklist 17-21항목 원조
>
> **calm-design 확장**: 33항목 (Phase 1.1)으로 확장 — 한국어 환경 4항목, 접근성 4항목, 폼 1항목, 환경·SEO 3항목, 성능 2항목, **Memorability 3항목 (Phase 1.1 추가)** 포함. WCAG 2.0 luminance 정밀 계산(`scripts/color-utils.py`) 통합. ⚠️ 부분 통과 단계 도입 (단순 ✅/❌ 이분법 회피).

calm-design이 어떤 출력이든 사용자에게 전달하기 직전 자동 실행하는 검증 게이트. 33항목 중 1개라도 ❌면 → `self-critique-loop`가 핀포인트 재생성.
...
### 🌟 카테고리 11: 기억에 남는 디자인 (Memorability, 3항목)

> **Phase 1.1 추가** — 체크리스트 기반 검증을 넘어 "이 디자인이 기억에 남는가?"를 평가하는 질적 항목. Vision 분석 필수.

#### 31. 기억할 시각적 시그니처 존재 (Memorability)
- ✅ 1주 후에도 기억할 고유 요소가 있음 (예: 토스의 블루, 당근의 오렌지, Linear의 퍼플 그래디언트)
  - 컬러 시그니처: 단일 액센트 색상이 브랜드처럼 기억됨
  - 레이아웃 시그니처: 독특한 Hero 구성 또는 비대칭 그리드
  - 타이포 시그니처: 큰 숫자, 인용문, 독특한 헤드라인 배치
  - 모션 시그니처: 기억에 남는 마이크로 인터랙션
- ⚠️ 깔끔하지만 특별히 기억나는 요소 없음 (범용적인 좋은 디자인)
- ❌ 어디서 많이 본 디자인 (AI 템플릿 느낌)
- 검증: **Vision 필수** — "이 화면에서 1주 후에도 기억할 요소가 있는가?" 질문

#### 32. 경쟁 차별화 시그니처 존재 (Differentiation)
- ✅ 같은 카테고리 경쟁 서비스와 즉시 구별됨
  - 색상 차별화: 경쟁사 대비 다른 색 계열
  - 톤 차별화: 경쟁사 대비 다른 분위기 (예: 경쟁사가 딱딱하면 친근하게)
  - 레이아웃 차별화: 경쟁사와 다른 정보 배치
- ⚠️ 차별화 요소 부족 (업계 표준 범위 내)
- ❌ 경쟁사와 너무 유사 (심지어 색상까지 비슷)
- 검증: **Vision + CONTEXT.md 참조** — `competitor_avoidance` 필드가 실제 반영되었는지

#### 33. "와" 순간 존재 (Delight Moment)
- ✅ 사용자가 "오" 또는 "와" 할 순간이 최소 1개 존재
  - Hero 진입 애니메이션
  - 독특한 마이크로 인터랙션 (버튼 호버, 스크롤 반응)
  - 예상치 못한 디테일 (404 페이지 디자인, 이스터에그)
  - 인상적인 데이터 시각화
- ⚠️ 기능적으로 완벽하지만 "와" 순간 부재
- ❌ 지루한 디자인 (기능만 있고 감정 자극 없음)
- 검증: **Vision + MOTION_INTENSITY 참조** — MOTION ≥ 6이면 필수

---

## Memorability 평가 기준 (0-10 스케일)

| 점수 | 의미 | 기준 |
|---|---|---|
| 9-10 | Exceptional | 즉시 기억, 공유하고 싶음, 업계 레퍼런스급 |
| 7-8 | Good | 기억에 남는 요소 있음, 차별화됨 |
| 5-6 | Adequate | 깔끔하지만 특별하지 않음, 범용적 |
| 3-4 | Weak | 어디서 본 듯한 디자인, 차별화 부족 |
| 1-2 | Poor | AI 템플릿 느낌, 즉시 잊힘 |

**통과 기준**: 31-33 항목 모두 ≥7점 (총 21점 이상 / 30점 만점)

---

## Phase 2+ 확장 예정 항목 (참고, 34+ 단계)

추후 추가 예정:
- 34. 색상만으로 정보 전달 부재 (색맹 대응)
- 35. 폼 에러 메시지 시각적 명시 (붉은 색상 + 아이콘 + 텍스트)
- 36. 이미지 lazy loading 명시 (`loading="lazy"`)
- 37. Critical CSS 인라인 (Phase 4 — preview-catalog 통합)
